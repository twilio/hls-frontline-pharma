...
exports.handler = async function (context, event, callback) {
  ...
  try {
    ...
    if (event.Anchor) {
      // workaround to avoid pagination
      response.setBody([]);
      ...
      switch (event.Location) {
        ...
        case "GetCustomersList": {
          if (event.Query && event.Query.length > 1) {
            response.setBody(
              await getCustomersSearch(event.Worker, event.Query, connection)
            );
          } else {
            response.setBody(
...
const getCustomersList = async (workerIdentity, connection) => {
  ...
  try {
    sfdcRecords = await connection
      ...
      .execute();
    console.log(
      "Fetched # SFDC records for customers list: " + sfdcRecords.length
    );
  } catch (err) {
    console.error(err);
  }
...
const getCustomersSearch = async (workerIdentity, query, connection) => {
  ...
  try {
    ...
    console.log(
      "Fetched # SFDC records for customers search: " +
        sfdcRecords.searchRecords.length
    );
  } catch (err) {
    console.error(err);
  }
